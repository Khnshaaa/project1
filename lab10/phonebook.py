...
# # Обновление данных
# def update_user():
#     old_name = input("Введите имя пользователя, которого хотите изменить: ")
#     new_name = input("Введите новое имя (Enter для пропуска): ")
#     new_phone = input("Введите новый номер (Enter для пропуска): ")
#     if new_name:
...
# def delete_user():
#     key = input("Введите имя или номер телефона для удаления: ")
...
#     cur.execute("SELECT * FROM Users WHERE user_name ILIKE %s OR user_phone LIKE %s", (f"%{filter_value}%", f"%{filter_value}%"))
...
#     while True:
#         print("\nМеню:")
#         print("1. Вставить данные (из консоли)")
